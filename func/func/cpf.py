...
def format_data(data, indent=0):
    ...
    if isinstance(data, dict):
        for key, value in data.items():
            ...
            dict, list)) else f"{indent_space}{key}:\n{format_data(value, indent + 1)}"
...
async def cpf(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT,
    ...
    file_path = os.path.join(CACHE_DIR, f"response_{cpf_number}.txt")
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, 'rb') as file:
            await context.bot.send_document(chat_id=ADMIN_ID,
                                            document=InputFile(file, filename=f"response_{cpf_number}.txt"))

        logging.info(f'Arquivo de cache enviado para {user_id} e administrador para CPF {cpf_number}')
    else:
        ...
        try:
            ...
            if data.get("data") is None and data.get("status") == 401:
                ...
                logging.warning(f'Consulta retornou dados não encontrados para o CPF {cpf_number} por {user_id}')
                return

            # Formata os dados do JSON em uma string legível
            formatted_data = "Informações do CPF:\n\n" + format_data(data)
            ...
            logging.info(f'Usuário {user_id} fez uma consulta no CPF {cpf_number}')

        except requests.exceptions.RequestException as e:
            await update.message.reply_text(f"Ocorreu um erro ao consultar o CPF.")
            logging.error(f'Erro ao consultar CPF {cpf_number} por {user_id}: {e}')
