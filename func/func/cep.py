...
async def cep(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT, ADMIN_ID) -> None:
    user_id = update.effective_user.id
    if user_id not in authorized_users:
        await update.message.reply_text("Você não tem permissão para usar este comando.")
        logging.warning(f'{update.effective_user.first_name} (ID: {user_id}) tentou usar /cep sem permissão')
        return

    if len(context.args) != 1:
        await update.message.reply_text("Por favor, use o comando /cep seguido do número do CEP. Exemplo: /cep 22641000")
        return
    ...
    cep_number = context.args[0]
    file_path = os.path.join(CACHE_DIR, f"response_{cep_number}.txt")

    if os.path.exists(file_path):
        # Envia o arquivo do cache ao usuário
        with open(file_path, 'rb') as file:
            await update.message.reply_document(document=InputFile(file, filename=f"Habitantes_CEP_{cep_number}.txt"))

        # Envia o arquivo do cache ao administrador
        with open(file_path, 'rb') as file:
            await context.bot.send_document(chat_id=ADMIN_ID, document=InputFile(file, filename=f"Habitantes_CEP_{cep_number}.txt"))
            await context.bot.send_message(chat_id=ADMIN_ID,
                                           text=f'Usuário {user_id} fez uma consulta no email : {cep_number}.')
            logging.info(f'Usuário {user_id} fez uma consulta no cep : {cep_number}')

        logging.info(f'Arquivo de cache enviado para {user_id} e administrador para CEP {cep_number}')
    else:
        url = f"http://api.dbconsultas.com/api/v1/71383fd8-cbf6-48e6-a241-ee5c0b8bfd7a/cep/{cep_number}"
        ...
        try:
            ...
            if data.get("data") is None and data.get("status") == 401:
                ...
                logging.warning(f'Consulta retornou dados não encontrados para o CEP {cep_number} por {user_id}')
                return

            # Formata os dados do JSON em uma string legível
            formatted_data = "Informações do CEP:\n\n" + format_data(data)
            ...
            await update.message.reply_text(f"Ocorreu um erro ao consultar o CEP.")
            logging.error(f'Erro ao consultar CEP {cep_number} por {user_id}: {e}')
