...
async def nome(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT, ADMIN_ID) -> None:
    ...
    try:
        ...
        file_path = f"Nome_{nome_codificado}.txt"
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f'Usuário {user_id} fez uma consulta no nome {nome_codificado}.')
        logging.info(f'Usuário {user_id} fez uma consulta no nome {nome_codificado}')

    except requests.exceptions.RequestException as e:
        await update.message.reply_text(f"Ocorreu um erro ao consultar o nome.")
        logging.error(f'Erro ao consultar nome {nome_codificado} por {user_id}: {e}')
