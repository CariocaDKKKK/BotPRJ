...
async def nome(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT,
               ADMIN_ID) -> None:
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, 'rb') as file:
            await update.message.reply_document(
                document=InputFile(file, filename=f"Informações do Nome: {nome_completo}.txt"))

        # Envia o arquivo do cache ao administrador
        with open(file_path, 'rb') as file:
            await context.bot.send_document(chat_id=ADMIN_ID, document=InputFile(file,
                                                                                 filename=f"Informações do Nome: {nome_completo}.txt"))
