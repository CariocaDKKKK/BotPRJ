...
async def cep(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT,
              ADMIN_ID) -> None:
    ...
    if len(context.args) != 1:
        await update.message.reply_text(
            "Por favor, use o comando /cep seguido do número do CEP. Exemplo: /cep 22641000")
        return
    ...
    if os.path.exists(file_path):
        ...
        with open(file_path, 'rb') as file:
            await context.bot.send_document(chat_id=ADMIN_ID,
                                            document=InputFile(file, filename=f"Habitantes_CEP_{cep_number}.txt"))
            await context.bot.send_message(chat_id=ADMIN_ID,
                                           text=f'Usuário {user_id} fez uma consulta no CEP: {cep_number}.')
