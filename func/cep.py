...
async def cep(update: Update, context: ContextTypes.DEFAULT_TYPE, authorized_users, user_usage, DAILY_LIMIT, ADMIN_ID) -> None:
    ...
    if len(context.args) != 1:
        await update.message.reply_text(
            "Por favor, use o comando /cep seguido do número do CPF. Exemplo: /cep 22641000")
        return
    ...
    try:
        ...
        if data.get("data") is None and data.get("status") == 401:
            ...
            logging.warning(f'Consulta retornou dados não encontrados para o nome {cep_number} por {user_id}')
            return

        # Formata os dados do JSON em uma string legível
        formatted_data = "Habitantes do CEP:\n\n" + format_data(data)

        # Salva a resposta formatada em um arquivo .txt
        file_path = f"CPF_{cep_number}.txt"
        with open(file_path, 'w', encoding='utf-8') as file:
            file.write(formatted_data)

        # Envia o arquivo ao usuário
        with open(file_path, 'rb') as file:
            await update.message.reply_document(document=InputFile(file, filename=f"Habitantes Cep : {cep_number}.txt"))
        ...
        await context.bot.send_message(chat_id=ADMIN_ID, text=f'Usuário {user_id} fez uma consulta no CEP {cep_number}.')
        logging.info(f'Usuário {user_id} fez uma consulta no CEP {cep_number}')

    except requests.exceptions.RequestException as e:
        await update.message.reply_text(f"Ocorreu um erro ao consultar o CPF.")
